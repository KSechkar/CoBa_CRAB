...
# import cell_genetic_modules as gms
...
class ModelAuxiliary:
    ...
    def plot_phys_variables(self, ts, xs,
                            par, synth_genes, synth_miscs, modules_name2pos,
                            module1_specterms, module2_specterms,
                            # inputs only used by the basic model - filled with Nones here
                            module1_F_calc=None, module2_F_calc=None,
                            uexprecord=None,
                            synth_genes_total_and_each=None,
                            synth_miscs_total_and_each=None,
    ...
    def get_e_l_Fr_nu_psi_T_D(self, t, x,
                              ...
                              modules_name2pos,
                              module1_specterms, module2_specterms,
                              # inputs only used by the basic model, here are Nones for consistency
                              module1_F_calc=None, module2_F_calc=None,
                              uexprecord=None,
                              synth_genes_total_and_each=None, synth_miscs_total_and_each=None
                              ):
        # get the state vector with effective mRNA concs (due to possible synth gene co-expression from same operons)
        module1_specterms_vmap = jax.vmap(module1_specterms, in_axes=(0, None, None))
        module2_specterms_vmap = jax.vmap(module2_specterms, in_axes=(0, None, None))
...
def odeuus(t, x, # simulation time and state vector
    ...
    dxdt = jnp.array([
                    ...
                    +
                    # add controller ODEs
                    controller_ode(t, x_eff, ctrl_memo, ref, e, l, R, D, par, modules_name2pos, controller_name2pos, ctrledvar))
...
def main():
    ...
    bkplot.save(bklayouts.grid([[mass_fig, nat_mrna_fig, nat_prot_fig],
                                ...
                                [e_figure, Fr_figure, ppGpp_figure, D_figure]]))
    ...
    # find the steady-state ribosomal mass fraction
    ss_phi_r=xs[-1,3]*par['n_r']/par['M']
    print('Steady-state ribosomal mass fraction: ', ss_phi_r)
